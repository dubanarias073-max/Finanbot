...
def get_cat(t):
    cat = t.categoria
    if cat is None:        return 'Sin categoría'
    if isinstance(cat, str): return cat
    if hasattr(cat, 'nombre'): return cat.nombre
    return str(cat)
...
def exportar_excel():
    ...
    for cat, monto in sorted(gastos_cat.items(), key=lambda x: x[1], reverse=True)[:10]:
        ws1[f'J{rg}'] = f'{get_icono(cat)} {cat}'
    ...
    for i, (cat, st) in enumerate(sorted(cat_stats.items(), key=lambda x: x[1]['total'], reverse=True), 6):
        ...
        rh(ws3, i, 19)
        wr(ws3, i, 1, f'{get_icono(cat)} {cat}', bg_, P['bl'], sz=9, h='left')
    ...
    for cat, st in sorted(cat_stats.items(), key=lambda x: x[1]['total'], reverse=True)[:8]:
        ws3[f'I{rc}'] = f'{get_icono(cat)} {cat}'
    ...
    # ════════════════════════════════════════════════════
    #  HOJA 5 — PLAN FINANCIERO 50/30/20 + PROYECCIONES
    # ════════════════════════════════════════════════════
    ws5 = wb.create_sheet('Plan Financiero')
    ws5.sheet_properties.tabColor = P['ve']
    cabecera(ws5, 'Plan Financiero Personal',
    ...
    if ing_base > 0:
        ...
        wr(ws5, 4, 1, '📋  DISTRIBUCIÓN RECOMENDADA (REGLA 50/30/20)',
        ...
        for i, (cat,pct_,mn_,an_,desc,real,col_) in enumerate(plan, 6):
            bg_ = P['ca'] if i%2==0 else P['bg']; rh(ws5, i, 24)
            wr(ws5,i,1,cat,            bg_,col_,bold=True,sz=10,h='left')
            wr(ws5,i,2,pct_,           bg_,col_,sz=9)
            wr(ws5,i,3,f'${mn_:,.0f}', bg_,P['bl'],bold=True,sz=10)
            wr(ws5,i,4,f'${an_:,.0f}', bg_,P['gr'],sz=9)
            wr(ws5,i,5,desc,           bg_,P['gr'],sz=9,h='left')
            wr(ws5,i,6,real,           bg_,P['cy'],sz=9)
        cw(ws5,1,24); cw(ws5,2,14); cw(ws5,3,16); cw(ws5,4,16); cw(ws5,5,30); cw(ws5,6,14)

        wr(ws5, 11, 1, '📈  PROYECCIONES (ahorrando el 20% del ingreso mensual)',
        ...
        for i,(m,lbl) in enumerate(horizontes, 13):
            ...
            bg_=P['ca'] if i%2==0 else P['bg']; rh(ws5,i,22)
            wr(ws5,i,1,lbl,                bg_,P['bl'],bold=True,sz=9)
            wr(ws5,i,2,f'${simple:,.0f}', bg_,P['gr'],sz=9)
            wr(ws5,i,3,f'${c8:,.0f}',    bg_,P['cy'],sz=9)
            wr(ws5,i,4,f'${c10:,.0f}',   bg_,P['ve'],bold=True,sz=10)
            wr(ws5,i,5,f'${c12:,.0f}',   bg_,P['am'],sz=9)
            wr(ws5,i,6,f'+${c10-simple:,.0f}',bg_,P['ve'],sz=9)

        ws5['H12']='Plazo'; ws5['I12']='Simple'; ws5['J12']='10% EA'
        for c in['H12','I12','J12']:
            ws5[c].fill=fl(P['li']); ws5[c].font=fn(P['bl'],bold=True,sz=9)
        cw(ws5,8,14); cw(ws5,9,14); cw(ws5,10,16)
        for i3,(m,lbl) in enumerate(horizontes,13):
            ws5[f'H{i3}']=lbl
            ws5[f'I{i3}']=round(n20*m,2)
            ws5[f'J{i3}']=round(ci(.10,m),2)
        ...
        dp=Reference(ws5,min_col=9,max_col=10,min_row=12,max_row=18)
