...
sesiones  = {}   # bot FinanBotIA por usuario
_contextos = {}  # contexto conversacional pendiente por usuario
# Estructura de _contextos[uid]:
#   {'esperando': 'monto'|'tasa'|'plazo'|'confirmacion',
#    'accion_pendiente': 'simulacion'|'gasto'|'meta'|...,
#    'datos': {monto, tasa, plazo, nombre, ...},
#    'ultimo_tipo': str  → tipo de la última acción ejecutada }


# ══════════════════════════════════════════════════════════════════
#  NORMALIZACIÓN DE TEXTO  ─  corrige typos comunes
# ══════════════════════════════════════════════════════════════════
_TYPOS = {
    r'\blataza\b':      'la tasa',
    r'\blatsa\b':       'la tasa',
    r'\blatasa\b':      'la tasa',
    r'\bel plazo\b':    'el plazo',
    r'\belplazo\b':     'el plazo',
    r'\belmonto\b':     'el monto',
    r'\bmesos\b':       'meses',
    r'\bmese\b':        'meses',
    r'\bañoss\b':       'años',
    r'\bsimulacion\b':  'simulación',
    r'\binversion\b':   'inversión',
    r'\btarjeta\b':     'tarjeta',
    r'\bregistra\b':    'registra',
    r'\bregistar\b':    'registra',
    r'\bborrar\b':      'borra',
    r'\bporcierto\b':   'por ciento',
    r'\bporciento\b':   'por ciento',
    r'\bdescuento\b':   'descuento',
    r'\biva\b':         'iva',
}

def _normalizar(texto: str) -> str:
    """Corrige typos frecuentes antes de procesar el mensaje."""
    t = texto
...
def mensaje():
    ...
    ctx    = _cargar_contexto(int(usuario_id)) if usuario_id else None
    uid    = int(usuario_id) if usuario_id else None
    accion = None

    # ── Intentar resolver contexto conversacional pendiente ──────
    if uid and ctx:
        accion = _resolver_contexto(msg_usuario, uid, ctx, session_key)

    # ── Si no hay continuación, ejecutar acción normal ───────────
    if accion is None and uid and ctx:
        accion = ejecutar_accion(msg_usuario, uid, ctx)
    ...
    # ── Agregar disclaimers según el tema ──────────────────────────
    ...
    return jsonify({
        ...
        'estado':           'ok',
        'es_ia':            True,  # Indicador de que es respuesta de IA
    }), 200


def _agregar_disclaimers_si_necesario(respuesta: str, mensaje: str) -> str:
    """Agregar disclaimers automáticos en temas sensibles."""
    msg = mensaje.lower()
    
    # Temas que requieren disclaimer de asesoría
...
# ══════════════════════════════════════════════════════════════════
#  CONTEXTO CONVERSACIONAL  ─  memoria entre turnos
# ══════════════════════════════════════════════════════════════════

def _actualizar_contexto(session_key: str, accion: dict | None, msg: str):
    """Guarda qué está esperando el bot en el próximo turno."""
    ctx_actual = _contextos.get(session_key, {})

    if accion is None:
        # Sin acción → limpiar contexto pendiente solo si no quedó esperando
        return

    tipo = accion.get('tipo', '')

    # Si el bot pidió un dato → guardar qué espera
    if tipo == 'pide_monto':
        contexto_accion = accion.get('contexto', '')
        _contextos[session_key] = {
            'esperando':       'monto',
            'accion_pendiente': contexto_accion,
            'datos':           ctx_actual.get('datos', {}),
        }
    elif tipo == 'simulacion_realizada':
        # Guardar los datos de la última simulación
        _contextos[session_key] = {
            'esperando':       None,
            'ultimo_tipo':     'simulacion',
            'datos': {
                'monto': accion.get('capital'),
        ...
        # Para otros tipos limpiar el estado de espera pero guardar último tipo
        _contextos[session_key] = {
            'esperando':   None,
            'ultimo_tipo': tipo,
            'datos':       {},
...
def _resolver_contexto(msg: str, uid: int, ctx: dict, session_key: str) -> dict | None:
    """
    Si hay un contexto pendiente (bot esperaba un dato), intenta resolverlo
    combinando lo que el usuario acaba de decir con los datos previos.
    También resuelve respuestas de seguimiento como 'sí', 'ese mismo', etc.
    """
    ...
    if esperando == 'monto' and accion_pendiente == 'simulacion':
        ...
        # ¿Tenemos suficiente para simular?
        if datos.get('monto'):
            ...
            try:
                ...
                db.session.add(Simulacion(
                    usuario_id=uid, capital_inicial=datos['monto'],
                    tasa_retorno=tasa_f, plazo_meses=plazo_f,
                    resultado_final=round(bal)
                ))
                db.session.commit()
            ...
            return {
                'tipo': 'simulacion_realizada', 'capital': datos['monto'],
                'tasa': tasa_f, 'plazo': plazo_f,
                'resultado': round(bal), 'ganancia': round(gan),
            }
        else:
            # Guardar lo que llegó parcialmente y seguir esperando
            _contextos[session_key]['datos'] = datos
            return {'tipo': 'pide_monto', 'contexto': 'simulacion'}

    # ── El bot esperaba monto para meta ──────────────────────────
    ...
    # ── El bot esperaba monto para gasto ─────────────────────────
    if esperando == 'monto' and accion_pendiente == 'gasto':
        ...
        if monto and monto > 0:
            ...
            try:
                ...
                c = (Categoria.query
                     .filter(Categoria.nombre.ilike(f'%{cat}%'), Categoria.tipo == 'gasto')
                     .first() or Categoria.query.filter_by(tipo='gasto').first())
                t = Transaccion(
                    usuario_id=uid, categoria_id=c.id if c else 1,
                    tipo='gasto', monto=monto,
                ...
                return {'tipo': 'gasto_registrado', 'monto': monto,
                        'categoria': c.nombre if c else 'Otros', 'id': t.id}
            except Exception as e:
                print(f'[FinanBot] Gasto contexto: {e}')

    # ── El bot esperaba monto para ingreso ───────────────────────
    if esperando == 'monto' and accion_pendiente == 'ingreso':
        ...
        if monto and monto > 0:
            ...
            try:
                from models import Transaccion, Categoria
                c = (Categoria.query
                     .filter(Categoria.nombre.ilike(f'%{cat}%'), Categoria.tipo == 'ingreso')
                ...
                return {'tipo': 'ingreso_registrado', 'monto': monto,
                        'categoria': c.nombre if c else 'Salario', 'id': t.id}
            except Exception as e:
                print(f'[FinanBot] Ingreso contexto: {e}')

    # ── Completar simulación con datos parciales del historial ───
    # Ej: bot preguntó cuánto, usuario dice "10000 la tasa 10% plazo 2 meses"
    # pero no activó el keyword 'simula' — detectar por números + tasa + plazo juntos
    if ultimo_tipo in ('simulacion', None) or esperando == 'monto':
        monto = extraer_monto(msg)
        tasa  = extraer_tasa(msg)
        plazo = extraer_plazo(msg)

        # Completar con datos previos
        monto = monto or datos_previos.get('monto')
        tasa  = tasa  or datos_previos.get('tasa')
        plazo = plazo or datos_previos.get('plazo')

        # Si hay monto Y (tasa O plazo) en el mensaje actual → es respuesta a simulación
        if monto and (extraer_tasa(msg) or extraer_plazo(msg)):
            ...
            try:
                from models import Simulacion
                db.session.add(Simulacion(
                    usuario_id=uid, capital_inicial=monto,
                    tasa_retorno=tasa_f, plazo_meses=plazo_f,
                    resultado_final=round(bal)
                ))
                db.session.commit()
            ...
            return {
                'tipo': 'simulacion_realizada', 'capital': monto,
                'tasa': tasa_f, 'plazo': plazo_f,
                'resultado': round(bal), 'ganancia': round(gan),
            }
...
def _cargar_contexto(uid: int) -> dict | None:
    try:
        transacciones = Transaccion.query.filter_by(usuario_id=uid).all()
        metas         = MetaAhorro.query.filter_by(usuario_id=uid).all()
        sims          = (Simulacion.query.filter_by(usuario_id=uid)
        ...
        return {
            ...
            'metas': [{
                ...
                'porcentaje': min(round(float(m.monto_actual)/float(m.monto_objetivo)*100), 100)
                              if m.monto_objetivo > 0 else 0,
...
CREAR = [
    'crea', 'crear', 'agrega', 'agregar', 'añade', 'añadir', 'añademe',
    'agregame', 'hazme', 'haz', 'ponme', 'pon', 'registra', 'registrar',
    'registrame', 'nueva', 'nuevo', 'generame', 'genera', 'ingresa', 'ingresame',
]
ELIMINAR = [
    'elimina', 'eliminar', 'borra', 'borrar', 'eliminame', 'quita', 'quitar',
    'quitame', 'suprime', 'suprimir', 'remueve', 'remover', 'bota', 'botar',
    'bórralo', 'bórrala',
]
ACTUALIZAR = [
    ...
    'abona', 'abonar', 'cambia a', 'cambiar a', 'ponme de', 'llámame',
    'llamame', 'mi nombre es', 'mi correo es',
]
CONSULTAR = [
    'cuantos', 'cuántos', 'cuanto', 'cuánto', 'revisa', 'revisar', 'revisame',
    'muestra', 'mostrar', 'muestrame', 'dime', 'cual', 'cuál', 'ver',
    'verifica', 'verifique', 'hay', 'tengo', 'lista', 'listar',
    'detalle', 'detalles', 'muéstrame',
]

META_KW      = ['meta', 'metas', 'objetivo', 'objetivos']
GASTO_KW     = ['gasto', 'gastos', 'gaste', 'gasté', 'compré', 'compre',
                 'pagué', 'pague', 'egreso', 'egresé']
INGRESO_KW   = ['ingreso', 'ingresos', 'salario', 'sueldo', 'cobré', 'cobre',
                 'gané', 'gane', 'recibí', 'recibi', 'quincena']
SIMULADOR_KW = ['simula', 'simulacion', 'simulación', 'simulador', 'simular']
PERFIL_KW    = ['nombre', 'correo', 'email', 'mi nombre', 'mi correo',
                 'llámame', 'llamame', 'salario mensual', 'ingreso mensual',
                 'meta mensual', 'meta_ahorro']
REPORTE_KW   = ['reporte', 'informe', 'exportar', 'descargar reporte',
                 'descargar informe', 'hazme un reporte', 'genera un reporte',
                 'quiero el reporte', 'pdf', 'excel']
CALCULO_KW   = ['suma', 'resta', 'multiplica', 'divide', 'calcula',
                 'cuanto es', 'cuánto es', 'cuanto son', 'cuánto son',
                 'cuanto vale', 'cuánto vale']
DESCUENTO_KW = ['descuento', 'rebaja', 'oferta', 'promocion', 'promoción',
                 'rebajado', 'con descuento', 'dto']
IVA_KW       = ['iva', 'con iva', 'más iva', 'mas iva', 'incluido iva', 'sin iva', 'impuesto']
REPARTO_KW   = ['dividir entre', 'dividido entre', 'repartir', 'repartimos',
                 'cuanto le toca', 'cuánto le toca', 'cuanto paga cada',
                 'cuánto paga cada']
AUMENTO_KW   = ['aumento', 'incremento', 'subio', 'subió', 'aumenta', 'incrementa']
PORCENTAJE_KW= ['que porcentaje', 'qué porcentaje', 'cuanto representa',
                 'cuánto representa', 'porcentaje de']
INTERES_KW   = ['interes', 'interés', 'cuanto gano', 'cuánto gano',
                 'cuanto genera', 'cuánto genera', 'rendimiento']
SALDO_KW     = ['cuanto me queda', 'cuánto me queda', 'me queda', 'que sobra',
                 'qué sobra', 'cuanto sobra', 'cuánto sobra']
BIENVENIDA_KW= ['hola', 'buenos dias', 'buenos días', 'buenas tardes',
                 'buenas noches', 'buenas', 'hey', 'saludos',
                 'que puedes hacer', 'qué puedes hacer', 'ayuda',
                 'como me ayudas', 'cómo me ayudas', 'que haces', 'qué haces',
                 'inicio', 'comenzar', 'empezar', 'para que sirves',
                 'para qué sirves']
BALANCE_KW   = ['balance', 'resumen', 'estado financiero', 'como estoy',
                 'cómo estoy', 'cuanto tengo', 'cuánto tengo', 'mis finanzas',
                 'como van mis', 'cómo van mis']
...
def ejecutar_accion(mensaje: str, uid: int, ctx: dict) -> dict | None:
    ...
    if _t(msg, REPORTE_KW):
        tipo_rep = 'pdf'
        if any(p in msg for p in ['excel', 'xlsx']):
            tipo_rep = 'excel'
    ...
    if _t(msg, SIMULADOR_KW):
        ...
        if monto and monto > 0:
            ...
            try:
                db.session.add(Simulacion(
                    usuario_id=uid, capital_inicial=monto,
                    tasa_retorno=tasa, plazo_meses=plazo,
                    resultado_final=round(bal)
                ))
                db.session.commit()
            except Exception as e:
        ...
        if u:
            ...
            if nombre and any(p in msg for p in ['nombre', 'llámame', 'llamame', 'mi nombre']):
                try:
                    ...
                    print(f'[FinanBot] Perfil nombre: {e}')
            ...
            if monto and any(p in msg for p in ['meta mensual', 'meta_ahorro', 'meta de ahorro mensual']):
                try:
                    ...
                    print(f'[FinanBot] Meta mensual: {e}')
    ...
    if _t(msg, META_KW):
        ...
        if _t(msg, ELIMINAR):
            ...
            if not encontrada:
                return {'tipo': 'confirmar_eliminar_meta',
                        'mensaje': 'No encontré esa meta. ¿Cuál quieres eliminar?',
                        'metas': metas}
            try:
                m = MetaAhorro.query.get(encontrada['id'])
        ...
        if _t(msg, ACTUALIZAR):
            ...
            if monto and metas:
                ...
                try:
                    m = MetaAhorro.query.get(meta['id'])
                    if m and m.usuario_id == uid:
    ...
    if _t(msg, GASTO_KW):
        ...
        if _t(msg, ELIMINAR):
            ...
            if not g:
                return {'tipo': 'confirmar_eliminar_gasto',
                        'mensaje': 'No encontré ese gasto. ¿Elimino el más reciente?',
                        'gastos': gastos[:5]}
            try:
                t = Transaccion.query.get(g['id'])
        ...
        if _t(msg, CONSULTAR):
            return {
                ...
                'total_gastos':        ctx.get('total_gastos', 0),
                'gastos_por_categoria': ctx.get('gastos_por_categoria', {}),
                'recientes': [t for t in ctx.get('transacciones_recientes', [])
                              if t['tipo'] == 'gasto'][:5],
            }
    ...
    if _t(msg, INGRESO_KW):
        ...
        if _t(msg, ELIMINAR):
            ...
            if not i:
                return {'tipo': 'confirmar_eliminar_ingreso',
                        'mensaje': 'No encontré ese ingreso. ¿Elimino el más reciente?',
                        'ingresos': ingresos[:5]}
            try:
                t = Transaccion.query.get(i['id'])
                if t and t.usuario_id == uid:
                    db.session.delete(t); db.session.commit()
