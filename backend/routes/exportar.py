...
def exportar_pdf():
    ...
    hd = Table([[Paragraph('FinanBot', S_TITULO), tabla_detalles]],
    ...
    meta_sub = f'{round(ahorrado/obj_total*100)}% de ${obj_total:,.0f}' if obj_total > 0 else 'Sin metas'
    ...
    tarjetas = Table([[
        ...
        tarjeta('BALANCE ACTUAL',   f'${balance:,.0f}',    bal_col, 'Balance positivo' if balance >= 0 else 'Balance negativo'),
    ...
    if ingresos_t > 0:
        ...
        if cat_mayor and gastos_t > 0:
            ...
            filas_af.append([
                Paragraph('Mayor categoria de gasto', S_TC),
    ...
    if gastos_cat:
        ...
        for cat, monto in sorted(gastos_cat.items(), key=lambda x: x[1], reverse=True):
            ...
            filas_cat.append([
                ...
                Paragraph(barra_progreso(pm, col), st('bc2', fontSize=7, textColor=col, fontName='Courier')),
            ])
    ...
    if metas:
        ...
        for i, m in enumerate(metas, 1):
            ...
            filas_mt.append([
                ...
                Paragraph(barra_progreso(int(pct_m), col_m),  st('mpb', fontSize=7, textColor=col_m, fontName='Courier')),
